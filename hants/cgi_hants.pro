...
PRO cgi_hants, ev, freqs=freqs, _EXTRA=extra, data_if=data_if, mask_if=mask_if, $
                   ...
                   interp_of=interp_of, log_file=log_file
  ...
  FOR i=0L, num_tiles-1 DO BEGIN
    ...
    FOR j=0, datasize[0]-1 DO BEGIN
      IF mask[j] EQ 1 THEN BEGIN
        result[j,*]=cgi_fftndvi(data[j,*], freqs=freqs, $
          status=tmp1, smNDVI=tmp2, iNDVI=tmp3, _EXTRA=extra)
        IF tmp1[0] EQ -1 THEN $
  ...
  IF unit2 GT 0 THEN BEGIN
    ...
    envi_setup_head, fname=status_of, ns=ns, nl=nl, nb=3, $
      ...
      map_info=map_info, bnames=['Nr. of retained data points', $
      'Nr. of iterations 1st loop','Nr. of iterations 2nd loop']
